...
- **Usage forwarding**: ACP usage components (input / output / cacheRead / cacheWrite) are forwarded to pi for cost/stat accounting (`BackendUsage`), but `usage.totalTokens` is deliberately rewritten so the footer percentage tracks the pi session rather than backend execution cost. ACP backends can perform multiple internal LLM calls in one pi turn (for example plan → tool → final answer), so their aggregate `totalTokens` is execution cost, not "current pi context size".
  - **Claude backend (`mode=piOccupancy`)**: the footer is `PiOccupancy = prefixOverhead + visibleTranscript`, where `prefixOverhead` (backend system prompt + tool definitions + project context + skills) is calibrated once per session signature and persisted as a JSONL-sibling sidecar (`<sessionFile>.acp-calibration.json`), so resume sessions get a stable native-pi-equivalent reading from the first turn. Calibration samples skip tool turns (they're an execution aggregate across plan/tool/final calls and would inflate the prefix). The signature is `(backend, modelId, cwd, bridgeConfigSignature, systemPromptAppendHash)` — change any of those (different model, different project, different MCP config, different appended system prompt) and the cached calibration is invalidated and re-measured on the next valid non-tool turn. A drift detector also re-measures silently when a fresh sample diverges from the cached value by more than 5,000 tokens or 15% (catches mid-session loads like CLAUDE.md edits, MCP server additions, or lazily loaded skills). The intent is that the footer answers the same question native pi answers — *"how close is this session to the point where the operator should compact or summarize?"* — at ~90% accuracy.
  - **Codex backend (`mode=visibleTranscriptOnly`)**: the footer is the `VisibleTranscript` estimate only (chars/4 over the pi session JSONL plus the current assistant output). Codex usage / cache semantics are not yet independently verified, so PiOccupancy calibration is intentionally claude-only. The codex footer will understate real LLM payload because it omits backend system prompt, tool definitions, and project context — this is honest underestimation, not silent wrong number.
  - **Diagnostic line**: every turn emits one line with explicit `mode=` and full calibration state — `[pi-shell-acp:usage] mode=piOccupancy backend=claude tokens=… components: overhead=… transcript=… calibration=cached(…)|fresh(…)|drift(delta=…,prev=…,new=…)|missing(reason=tool_turn|no_valid_sample_yet)|stale(reason=…)|rejected(reason=…) backendRaw: input=… cacheRead=… …`. This is the only state surface in the bridge; no TUI changes.
