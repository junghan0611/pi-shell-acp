...
Permission mode and the CLAUDE_CONFIG_DIR overlay: `claude-agent-acp` resolves the SDK's top-level `Options.permissionMode` from its own `SettingsManager` read of `~/.claude/settings.json`'s `permissions.defaultMode` (independent of `Options.settingSources`), and pi-shell-acp cannot override that resolution via `_meta` (the agent hardcodes the resolved value after spreading user-provided options). To prevent the operator's native default mode (often `"auto"`) from leaking into pi-shell-acp sessions — and triggering Claude Code's auto-mode classifier inside the bridge — pi-shell-acp launches `claude-agent-acp` with `CLAUDE_CONFIG_DIR` pointed at a pi-owned overlay directory at `~/.pi/agent/claude-config-overlay/`. The overlay contains a minimal `settings.json` authored by pi-shell-acp (currently pinning `permissions.defaultMode` to `"default"`) plus a symlink for every other entry in `~/.claude/` (credentials, projects, agents, plugin cache, skills, etc.). The overlay is rebuilt idempotently on every claude session bootstrap, so new entries appearing in `~/.claude/` later surface on the next launch. An operator who explicitly exports `CLAUDE_CONFIG_DIR` in their shell keeps full control — process env wins over the bridge default.
