...
export default function (pi: ExtensionAPI) {
  ...
  const entwurfModeSchema = Type.Union(
  ...
  const entwurfParameters = Type.Object({
    ...
    cwd: Type.Optional(
      Type.String({ description: "Working directory for the entwurf" }),
    ),
    provider: Type.Optional(
      Type.String({ description: "Provider id (e.g. 'pi-shell-acp', 'openai-codex'). Pair with `model` to disambiguate against the Entwurf Target Registry." }),
    ),
    model: Type.Optional(
  ...
  registerTool({
    ...
    parameters: Type.Object({
      taskId: Type.Optional(
        Type.String({ description: "Specific entwurf task ID. Omit to list all." }),
      ),
    }),
