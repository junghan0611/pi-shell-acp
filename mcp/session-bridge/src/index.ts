      ...
      while (idx !== -1) {
        ...
        try {
          ...
          if (cmd.type === "send" && cmd.message) {
            ...
            const resp: RpcResponse = { type: "response", command: cmd.type || "unknown", success: false, error: "Unsupported command" };
            conn.write(JSON.stringify(resp) + "\n");
          }
        } catch {
...
async function resolveTarget(sessionId?: string, sessionName?: string): Promise<{ socketPath: string; display: string } | { error: string }> {
  if (sessionName) {
