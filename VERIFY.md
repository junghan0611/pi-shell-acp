...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'ok만 답하세요'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p '현재 날짜와 시간을 알려주세요. date 명령어를 사용하세요.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '이 세션의 암호문은 blue-otter-913 입니다. 설명 없이 READY만 답하세요.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '방금 내가 준 암호문만 그대로 답하세요.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '이제 암호문을 red-otter-204로 바꾸자. CHANGED만 답하세요.'
```

```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '현재 암호문만 답하세요.'
```
...
export SESSION_FILE=$(mktemp /tmp/pi-shell-acp-verify-XXXXXX.jsonl)
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'Remember this exact secret token for later: test-token-123. Reply only READY.'
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'What was the secret token? Reply with the token only.'
```
...
export SESSION_FILE=$(mktemp /tmp/pi-shell-acp-verify-XXXXXX.jsonl)
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '세션 보존 테스트입니다. READY만 답하세요.'
find "$CACHE_DIR" -maxdepth 1 -type f | sort
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '직전 테스트가 무엇이었는지 한 줄로 답하세요.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'README.md 첫 3줄을 읽고 한 줄로 요약해줘.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p '이 프로젝트에서 extractPromptBlocks 함수가 정의된 파일을 grep으로 찾아줘.'
```
...
```bash
cd "$PROJECT_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p '현재 git 브랜치와 최근 커밋 메시지 1개를 알려줘.'
```
...
```bash
cd "$REPO_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'delegate 툴이 보이면, 아주 짧은 sync 위임을 1회 실행해줘. 보이지 않으면 정확히 "delegate tool not visible"이라고만 답해.'
```
...
```bash
cd "$REPO_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'session_search 또는 knowledge_search 툴이 보이면 각각 가능 여부를 짧게 말해줘. 둘 다 안 보이면 정확히 "pi custom tools not visible"이라고만 답해.'
```
...
```bash
cd "$REPO_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'AGENTS.md와 README.md를 읽고, 이 리포의 현재 불변식만 7줄 이내로 요약해줘. 특히 provider/model/settings 이름, session continuity 경계, bootstrap 순서, 하지 말아야 할 것을 포함해.'
```
...
```bash
cd "$REPO_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'acp-bridge.ts와 index.ts를 기준으로, 지금 pi-shell-acp의 핵심 구조를 설명해줘. agent-shell을 semantic reference로 보되, 우리가 일부러 가져오지 않은 것까지 함께 말해줘.'
```
...
```bash
cd "$REPO_DIR"
pi -e "$REPO_DIR" --provider pi-shell-acp --model claude-sonnet-4-6 -p '현재 상태에서 다음 고도화 포인트를 3개만 제안해줘. 단, 얇은 bridge 원칙을 깨지 않는 것만. 각 항목마다 왜 필요한지, 어느 파일을 건드리는지, 검증 방법을 붙여줘.'
```
...
export SESSION_FILE=$(mktemp /tmp/pi-shell-acp-verify-XXXXXX.jsonl)
cd "$REPO_DIR"
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p 'AGENTS.md를 읽고 이 리포의 핵심 원칙 3개만 답해.'
pi -e "$REPO_DIR" --session "$SESSION_FILE" --provider pi-shell-acp --model claude-sonnet-4-6 -p '방금 답변을 한 줄로 다시 요약해.'
```
